...
// Pages worth reading beyond the homepage — where boarding / webcam / vet info lives.
const PAGE_HINTS =
  /\b(board|boarding|kennel|lodging|overnight|suites?|daycare|day-care|dog-?day|service|services|amenities|facilit|webcam|web-?cam|live-?cam|puppy-?cam|pet-?cam|camera|stream|watch|about|team|staff|pricing|rates|reservation|book)\b/i;

// Keyword signals we surface to the model as hints (it must still verify against the text).
const SIGNAL_SETS = {
  webcam: /\b(web ?cams?|live ?cams?|puppy ?cams?|pet ?cams?|live ?stream(?:ing)?|live ?video|watch your (?:dog|pet|cat|puppy)|view your (?:dog|pet)|camera access|24\/?7 cameras?)\b/gi,
...
// pull page URLs out of sitemap.xml (and one level of sitemap index)
async function sitemapUrls(baseUrl) {
  const roots = ['/sitemap.xml', '/sitemap_index.xml', '/sitemap-index.xml'];
  const found = new Set();
  for (const path of roots) {
    ...
    // if it's an index of sitemaps, fetch the most relevant child sitemap
    if (/<sitemapindex/i.test(r.body)) {
      const child = locs.find((l) => PAGE_HINTS.test(l)) || locs[0];
      if (child) {
        const cr = await getRaw(child, 8000).catch(() => null);
        if (cr?.ok) [...cr.body.matchAll(/<loc>\s*([^<\s]+)\s*<\/loc>/gi)].forEach((m) => found.add(m[1]));
      }
    } else {
  ...
  return [...found].filter((u) => sameHost(u, baseUrl));
}
...
function scoreUrl(url, text) {
  ...
  if (/board|kennel|overnight|lodg/i.test(url + ' ' + text)) s += 2;
  if (/webcam|web-?cam|live-?cam|camera|stream/i.test(url + ' ' + text)) s += 2;
  if (/service|amenit|facilit/i.test(url + ' ' + text)) s += 1;
  return s;
}
...
export async function gatherSiteText(siteUrl, { maxPages = 7, maxChars = 16000 } = {}) {
  ...
  for (const f of fetched) if (f.r.ok && /html|xml/i.test(f.r.ctype || 'text/html')) parts.push({ url: f.url, text: htmlToText(f.r.body) });
  ...
  text = text.trim();
  return { ok: true, finalUrl: baseUrl, pages: parts.length, pageUrls: parts.map((p) => p.url), text, signals: computeSignals(text) };
}
