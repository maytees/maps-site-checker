...
const PAGE_HINTS =
  /\b(board|boarding|kennel|lodging|overnight|suites?|daycare|day-care|dog-?day|service|services|amenities|facilit|webcam|web-?cam|live-?cam|puppy-?cam|pet-?cam|camera|stream|watch|about|team|staff|leadership|founders?|owners?|management|people|meet|our-?team|pricing|rates|reservation|book|contact|location|locations)\b/i;
...
export async function gatherSiteText(siteUrl, { maxPages = 8, maxChars = 16000, pick } = {}) {
  ...
  for (const u of await sitemapUrls(baseUrl)) {
    ...
    cand.set(key, { url: u, text: '', score: scoreUrl(u, '') }); // keep ALL sitemap pages as candidates (let the AI decide)
  }

  const ranked = [...cand.values()].sort((a, b) => b.score - a.score);

  // Choose which inner pages to read. If a `pick` fn is given (AI page-picker),
  // it chooses from a regex-ranked shortlist; otherwise fall back to regex top-N.
  let chosen = null;
  if (pick) {
    const shortlist = ranked.slice(0, 50); // regex narrows huge sites so the AI prompt stays small
    try {
      const picked = await pick(shortlist.map((c) => ({ url: c.url, hint: c.text || '' })));
      if (Array.isArray(picked) && picked.length) {
        const valid = new Set(ranked.map((c) => strip(c.url)));
        chosen = picked.filter((u) => valid.has(strip(u)));
      }
    } catch { /* fall through to regex */ }
  }
  if (!chosen || !chosen.length) chosen = ranked.filter((c) => c.score > 0).map((c) => c.url);
  chosen = chosen.slice(0, maxPages - 1);

  // fetch the chosen pages in parallel (homepage already fetched)
  const fetched = await Promise.all(chosen.map((u) => getRaw(u, 9000).then((r) => ({ url: u, r }))));
