...
export async function classify({ model, checks, instruction, businessName, text, signals }) {
  const keys = checks.map((c) => c.key);
  const checkList = checks.map((c) => `- "${c.key}": ${c.question}`).join('\n');

  const schema =
    '{' +
    checks.map((c) => `"${c.key}":"yes|no|unclear"`).join(',') +
    ',"business_type":"short label e.g. boarding kennel / pet hotel / vet / solo pet sitter / groomer / daycare","confidence":"high|medium|low","notes":"one short sentence citing the evidence and which page"}';
  ...
  const sys =
    ...
    'Answer each question "yes", "no", or "unclear". Use "unclear" only when the text truly does not say. ' +
    'Definitions: ' +
