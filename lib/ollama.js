...
export async function chatJson({ model, system, user, numCtx = 8192, tag = 'llm', timeoutMs = 60000 }) {
  ...
  try {
    ...
    err(tag, e.name === 'AbortError' ? `timeout after ${timeoutMs}ms` : `unreachable: ${String(e.message || e)}`);
    return null;
  } finally {
    clearTimeout(t);
  }
