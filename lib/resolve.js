...
// Poll until the captcha page is gone (you solved it), keeping the browser alive.
// Returns true once cleared, false on timeout / window closed.
async function waitForSolve(page, mapsUrl, ms) {
  const deadline = Date.now() + ms;
  while (Date.now() < deadline) {
    try { await page.waitForTimeout(1500); } catch { return false; } // window closed
    touchIdle(); // don't auto-close the browser while you're solving
    let u;
    try { u = page.url(); } catch { return false; }
    if (!/\/sorry\//i.test(u)) {
      if (!/\/maps\//i.test(u)) { try { await page.goto(mapsUrl, { waitUntil: 'domcontentloaded', timeout: 25000 }); } catch { /* ignore */ } }
      return true;
    }
  }
  return false;
...
async function runResolve(mapsUrl) {
  ...
  touchIdle();
  const headful = G.__msc.mode === 'chrome';
  const page = await ctx.newPage();
  try {
    // block heavy assets in headless (faster). In real-Chrome mode load everything,
    // so a captcha actually renders for you to solve.
    if (!headful) {
    ...
    if (/\/sorry\//i.test(page.url())) {
      if (headful) {
        // visible window — bring it up and wait for YOU to solve the captcha, then continue
        await page.bringToFront().catch(() => {});
        log('resolve', '⏸  captcha — solve it in the Chrome window; waiting up to 5 min…');
        const solved = await waitForSolve(page, mapsUrl, 300000);
        if (!solved) return { status: 'blocked', error: 'captcha not solved in time', businessStatus: '' };
        log('resolve', '▶  captcha solved, continuing');
      } else {
        return { status: 'blocked', error: 'Google captcha — slow down / try later', businessStatus: '' };
      }
    }
